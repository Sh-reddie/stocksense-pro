    ...
    try {
      ...
      if (r.ok) {
        ...
        if (meta?.regularMarketPrice) {
          // Convert chart meta → quoteResponse shape
          return {
            symbol: sym,
...
export async function onRequestGet({ request }) {
  ...
  // Short edge-cache for live prices — 15s is enough to deduplicate
  // burst requests (multiple tabs, auto-refresh) without serving stale prices.
  const PRICE_CACHE = { ...CORS, 'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=15, s-maxage=15' };
