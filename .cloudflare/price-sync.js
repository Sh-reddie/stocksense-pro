...
async function checkAndSendAlerts(env,prices){let portfolio=null;try{const raw=await env.STOCKSENSE_KV.get('portfolio');if(raw)portfolio=JSON.parse(raw);}catch(e){return;}if(!portfolio)return;const cfg=portfolio.cfg||{};const token=cfg.tgToken;const chatId=cfg.tgChatId;if(!token||!chatId){console.log('no tg creds in cfg');return;}let cool={};try{const raw=await env.STOCKSENSE_KV.get('alertCooldowns');if(raw)cool=JSON.parse(raw);}catch(e){}const now=Date.now();const COOL=3600000;let dirty=false;const canFire=k=>!cool[k]||(now-cool[k])>=COOL;const fired=k=>{cool[k]=now;dirty=true;};const msgs=[];for(const h of(portfolio.holdings||[])){if(!h.symbol)continue;const xch=h.exchange||'NSE';const cached=prices[h.symbol+'|'+xch];if(!cached)continue;const ltp=cached.ltp,cp=cached.chgP;if(h.stopLoss&&ltp>0&&ltp<h.stopLoss&&canFire('sl|'+h.id)){const pct=((h.stopLoss-ltp)/h.stopLoss*100).toFixed(1);msgs.push('\u{1f6a8} <b>SL BREACH: '+h.symbol+'</b>\nLTP \u20b9'+ltp.toFixed(2)+' below SL \u20b9'+h.stopLoss.toFixed(2)+' (-'+pct+'%)\nConsider cutting position.');fired('sl|'+h.id);}if(h.target1&&ltp>=h.target1&&canFire('t1|'+h.id)){const pp=h.avgPrice?((ltp-h.avgPrice)/h.avgPrice*100).toFixed(1):'?';msgs.push('\u{1f3af} <b>TARGET 1 HIT: '+h.symbol+'</b>\nLTP \u20b9'+ltp.toFixed(2)+' reached T1 \u20b9'+h.target1.toFixed(2)+' (+'+pp+'% from avg)\nConsider partial booking.');fired('t1|'+h.id);}if(h.target2&&ltp>=h.target2&&canFire('t2|'+h.id)){const pp=h.avgPrice?((ltp-h.avgPrice)/h.avgPrice*100).toFixed(1):'?';msgs.push('\u{1f3c6} <b>TARGET 2 HIT: '+h.symbol+'</b>\nLTP \u20b9'+ltp.toFixed(2)+' reached T2 \u20b9'+h.target2.toFixed(2)+' (+'+pp+'% from avg)\nConsider full exit.');fired('t2|'+h.id);}if(Math.abs(cp)>=5&&canFire('mv|'+h.id+'|'+new Date().toDateString())){msgs.push((cp>0?'\u{1f4c8}':'\u{1f4c9}')+' <b>'+h.symbol+' '+cp.toFixed(1)+'%</b>\nLTP \u20b9'+ltp.toFixed(2)+' \u2014 big intraday move in your portfolio.');fired('mv|'+h.id+'|'+new Date().toDateString());}}for(const a of(portfolio.alerts||[])){if(a.triggered||!a.sym||!a.price)continue;const ltp=((prices[a.sym+'|NSE']||prices[a.sym+'|BSE'])||{}).ltp||0;if(!ltp)continue;const hit=(a.type==='above'&&ltp>=a.price)||(a.type==='below'&&ltp<=a.price);if(hit&&canFire('ca|'+a.id)){msgs.push('\u{1f514} <b>ALERT: '+a.sym+'</b>\nLTP \u20b9'+ltp.toFixed(2)+' '+(a.type==='above'?'crossed above':'crossed below')+' \u20b9'+a.price.toFixed(2)+(a.label?' ('+a.label+')':''));fired('ca|'+a.id);}}for(const m of msgs){await tgSend(token,chatId,m);await new Promise(r=>setTimeout(r,300));}if(msgs.length)console.log('sent',msgs.length,'alert(s) to Telegram');if(dirty){for(const k of Object.keys(cool))if(now-cool[k]>86400000)delete cool[k];await env.STOCKSENSE_KV.put('alertCooldowns',JSON.stringify(cool));}}
...
async function sendMorningBrief(env) {
  ...
  // Near SL (within 3%) \u2014 not yet breached
  const nearSL = [];
...
async function handleTelegram(request,env){let update;try{update=await request.json();}catch(e){return new Response('ok');}const msg=update.message||update.edited_message;if(!msg)return new Response('ok');const chatId=msg.chat&&msg.chat.id;const text=(msg.text||'').trim();if(!chatId)return new Response('ok');let token=null;try{const raw=await env.STOCKSENSE_KV.get('portfolio');if(raw)token=(JSON.parse(raw)).cfg?.tgToken;}catch(e){}if(!token)return new Response('ok');const cmd=text.split(' ')[0].toLowerCase().replace(/@.*$/,'');if(cmd==='/portfolio'||cmd==='/p'){await sendPortfolioSnapshot(env,chatId,token);}else if(cmd==='/brief'||cmd==='/morning'){await sendMorningBrief(env);}else if(cmd==='/help'||cmd==='/start'){await tgSend(token,chatId,'<b>StockSense Pro Bot</b>\n\nCommands:\n/portfolio (or /p) \u2014 live portfolio snapshot with P&amp;L\n/brief \u2014 morning brief on demand\n\nAutomatic every 15 min during market hours:\n\u2022 Stop loss breached\n\u2022 Target 1 / Target 2 hit\n\u2022 Custom price alerts triggered\n\u2022 Stock moves \u22655% intraday\n\nAutomatic at 9 AM IST every weekday:\n\u2022 Morning brief (earnings today, near-SL, near-T1)');}return new Response('ok');}

export default{async fetch(request,env,ctx){const url=new URL(request.url);if(url.pathname==='/telegram'){if(request.method==='POST')return handleTelegram(request,env);return new Response('StockSense Telegram webhook',{status:200});}if(url.pathname==='/sync'){const{fetched,prices}=await syncPrices(env);ctx.waitUntil(checkAndSendAlerts(env,prices));return new Response(JSON.stringify({fetched}),{headers:{'Content-Type':'application/json'}});}if(url.pathname==='/brief'){ctx.waitUntil(sendMorningBrief(env));return new Response('{"ok":true}',{headers:{'Content-Type':'application/json'}});}return new Response('StockSense Price Sync Worker v3',{status:200});},async scheduled(event,env,ctx){ctx.waitUntil((async()=>{if(event.cron==='25 3 * * 1-5'){await sendMorningBrief(env);}else{const{prices}=await syncPrices(env);await checkAndSendAlerts(env,prices);}})());}};
